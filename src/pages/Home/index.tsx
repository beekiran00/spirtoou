...
import { modeTypes } from "./queryModeTypes";
      ...
      >
        <div style={{ display: "flex", flex: 0.2 }}>
          ...
          >
            ...
            >
              {/* {modeTypes.map((item: any, pos: number) => {
                return (
                  <>
                    <input
                      key={randomString(5)}
                      type="radio"
                      style={{ marginLeft: 10 }}
                      value={modeTypes[pos]?.mode}
                      defaultChecked={modeTypes[pos]?.defaultChecked}
                    />
                    {modeTypes[pos]?.radioBtnlabel}
                  </>
                );
              })} */}
              <input
          ...
          >
            ...
            >
              <p style={{ fontSize: 12, textAlign: "center" }}>
                Query examples for test
              </p>

              {querySamples.map((item: any, pos: number) => {
                return (
                  <button
                    key={randomString(5)}
                    onClick={() => {
                      setSearchTermsArray(querySamples[pos]?.queryText);
                    }}
                    style={{ margin: "5px 0px" }}
                  >
                    {querySamples[pos]?.queryType}
                  </button>
                );
              })}
            </div>
